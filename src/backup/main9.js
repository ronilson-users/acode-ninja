...
class AcodeConsole {
  ...
  addToLogger(message) {
    ...
    // Criar um elemento <pre> para preservar a formatação
  ...
  /**
   * Destroi o plugin e limpa os logs.
   */
  async destroy() {
    this.isRunning = false;
    this.clearLogs();
...
/**
 * Função para testar uma requisição fetch para a API pública JSONPlaceholder.
 */
...
// Chama a função para testar a requisição fetch
fetchTest();