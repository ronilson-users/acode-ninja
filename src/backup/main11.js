...
class AcodeConsole {
 ...
 addOutput(aceLineIdx, code) {
  ...
  consoleOutput.className = "ace_outupts-console";
...
highlightJson(jsonString) {
    return jsonString
      .replace(/(".*?"):/g, '<span style="color: #55aaff">$1</span>:') // Keys in blue
      .replace(/: (".*?")/g, ': <span style="color: #ffaa55">$1</span>') // Strings in orange
      .replace(/: (\d+\.?\d*)/g, ': <span style="color: #55ff55">$1</span>') // Numbers in green
      .replace(/: (true|false)/g, ': <span style="color: #ff5555">$1</span>') // Booleans in red
      .replace(/: (null)/g, ': <span style="color: #aaa">$1</span>'); // Null in gray
  }
