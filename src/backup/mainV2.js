...
// Network Logging
 createLoggerView() {
  ...
  // Container de logs (adicionado aqui)
  ...
  // Adiciona o loggerView ao documento
 ...
 async init(baseUrl) {
  ...
  // Atualizar posições conforme necessário
});
...
background-image: url('data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 30 30"><circle cx="12" cy="12" r="8" fill="%23ffcc00"/></svg>');
...
`;
  ...
  this.editor.renderer.$gutter.addEventListener("click", function (event) {
  const target = event.target;
  
  if (target.classList.contains("console-log-marker")) {
    console.log("Clicou na marcação do console.log!");
    // Aqui você pode adicionar a lógica desejada, como abrir um painel ou exibir logs
  }
});
